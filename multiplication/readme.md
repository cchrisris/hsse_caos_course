...
Вам предстоит реализовать функцию `Multiply`, которая перемножает 2 числа, в файле `multiplication.hpp`.
...
* `*.hpp` --- некоторый .hpp файл, в котором вы должны реализовать то, что требуется в условии задачи (в данном случае `multiplication.hpp`).
...
Для корректной сборки необходим `g++` версии не ниже 12 или `clang++` версии не ниже 18 (лучше самую новую).
