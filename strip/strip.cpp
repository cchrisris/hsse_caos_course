 ...
 public:
  ...
  void Process() {
    ...
    if (rename(new_file_path.data(), filename.data()) < 0) {
      throw std::runtime_error("failed to replace file, errno" +
                               std::to_string(errno));
    }
 ...
 private:
  ...
  void ProcessProgramHeaders(Elf* og_elf, Elf* new_elf) {
    ...
    if (elf_getphdrnum(og_elf, &phnum) < 0) {
      throw std::runtime_error(
          "failed to get the number of program headers in the elf file");
    }
  ...
  void ProcessSectionHeaders(Elf* og_elf, Elf* new_elf) {
    ...
    while ((scn = elf_nextscn(og_elf, scn)) != nullptr) {
      ...
      if (!new_scn) throw std::runtime_error("elf_newscn failed");
    ...
    if (rem) next_offset_nonalloc += (sh_align - rem);
